...
func (self *Client) Connect(uri string) (err error) {
	var URL *url.URL
	...
	dailer := net.Dialer{Timeout: self.RtspTimeout}
	var conn net.Conn
...
func (self *Client) SendRtpKeepalive() (err error) {
	if time.Now().Sub(self.rtpKeepaliveTimer) > self.RtpKeepAliveTimeout {
...
func (self *Client) KeepAliveOptions() (err error) {
	req := Request{
		Method: "OPTIONS",
		Uri:    self.requestUri,
	}
...
func (self *Client) Options() (err error) {
	...
	for i := 0; i < 2; i++ {
		...
		if res.StatusCode == 200 {
			...
			err = fmt.Errorf("rtsp: Options failed, StatusCode=%d", res.StatusCode)
		}
	}
	return
}
...
func (self *Stream) handlePacket(timestamp uint32, packet []byte) (err error) {
	...
	case av.H264:
		...
		*/
		...
		case naluType == 24:
			//2016/06/14 14:05:01      [103 66 0 40 233 0 160 11 117 196 128 3 110 232 0 205 254 96 13 136 16 148]       [104 206 49 82]
			//            [120] [0 22] [103 66 0 40 233 0 160 11 117 196 128 3 110 232 0 205 254 96 13 136 16 148] [0 4] [104 206 49 82]
			//              T    LEN   PAYLOAD SPS                                                                  LEN  PAYLOAD PPS
			//
			//sample need redisign not safe
			nalu_size_sps := (packet[1] << 8) | packet[2]
			nalu_size_pps := (packet[3+nalu_size_sps] << 8) | packet[4+nalu_size_sps]
			self.sps = packet[3 : nalu_size_sps+3]
			self.pps = packet[nalu_size_sps+3+2 : nalu_size_sps+3+2+nalu_size_pps]
			//no error STAP-A splited sps / pps packet
			//err = fmt.Errorf("rtsp: unsupported H264 STAP-A", packet)
			return
...
func New() (cli *Client) {
	...
	_cli.RtpKeepAliveTimeout = time.Duration(20 * time.Second)
	_cli.RtspTimeout = time.Duration(2 * time.Second)
	_cli.RtpTimeout = time.Duration(10 * time.Second)
	_cli.Headers = []string{"User-Agent: Lavf57.8.102"}
	cli = _cli
	return
}
func (self *Client) Open(uri string) (err error) {
	if err = self.Connect(uri); err != nil {
		return
	}
	if err = self.Options(); err != nil {
		return
